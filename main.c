...
/* Funktion til at vise indholdet af en fil */
...
/* Funktion til at redigere indholdet af en fil */
void editFileContents(char *filename) {
    FILE *file = fopen(filename, "a+");  // Åbn filen i "a+"-tilstand for at tilføje
    if (file == NULL) {
        printf("Kunne ikke åbne filen '%s'\n", filename);
        return;
    }
    ...
    while (1) {
        ...
        if (ch == 19) {  // Ctrl+S for at gemme (Ctrl og bogstavet 'S')
            ...
            if (fgets(modifiedContents, sizeof(modifiedContents), stdin) == NULL) {
                break;  // Afslut når der stødes på EOF (f.eks. Ctrl+D)
            }
            fprintf(file, "%s", modifiedContents);  // Tilføj den nye tekst
        }
    }
}

/* Funktion til at oprette en ny fil */
...
/* Funktion til at vise ændringstidspunktet for en fil */
void showFileModificationTime(char *filename) {
    ...
    if (stat(filename, &attrib) == 0) {
        ...
        printf("Filens ændringstidspunkt: %s\n", mod_time_str);
    } else {
